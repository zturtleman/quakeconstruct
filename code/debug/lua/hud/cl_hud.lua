...
local function draw2d()
	...
	local pkn = "<invalid>"
		...
		y = hud_field(y,hp,true,"Health",{dhp2,dhp,0,1},30)
		if(hp > 0) then
			y = hud_field(y,armor,true,"Armor",{.7,1,0,1},20)
			y = hud_field(y,ammo,true,"Ammo",{1,1,.2,1},20)
		end
		y = hud_field(y,util.GetWeaponName(_CG.weapon),false,nil,{1,1,1,1},12)
		if((pk + 3000) > LevelTime() and pkn != nil) then
			local dp = ((pk+3000) - LevelTime())/3000
			y = hud_field(y,pkn,false,nil,{1,1,1,dp},15)
		end
	hud_drawPickups()
	hud_drawFlashes()
	y=470
